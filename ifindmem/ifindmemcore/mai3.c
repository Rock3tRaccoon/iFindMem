...
void interact(pid_t pid, mach_port_t task) {
    ...
    while (1) {
        ...
        //search_read (2 args)
            ...
            search_data(task, false, false, base, end, (vm_address_t **)&out, &resultnum, args[1]);
        }
        ...
        //search_read (2 args)
            ...
            printf(GOOD"Memory Range = 0x%lx - 0x%lx\n", base, end);
            printf(GOOD"Finding bytes - %s\n", args[1]);
            search_data(task, false, true, base, end, (vm_address_t **)&out, &resultnum, args[1]);
        }
...
int main(int argc, char *argv[]) {
    ...
    //check if root
    if (geteuid() && getuid()) {
        printf(ERROR"Run iFindMemory as root.\n");
        EXIT
    }
